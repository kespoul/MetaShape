# -*- coding: utf-8 -*-
"""
Created on Thu Mar 21 13:54:39 2024

@author: kiesan

Description: This example calculates the necessary phase profile to map a lambertian light source to a
constant intensity profile at the target plane while also performing a slight focus og the beam (in 1D). It then
...
Tx = 0.5*X  # [mm] Target plane width
N = 20000   # Number of pixels. Should be X/(metasurface period)
...
# Input: Lambartian distribution
theta = np.arctan(x_init/z) # Beam incidence angle
I0 = 10
I = 4*I0*(z/sqrt(z**2+x**2))*(1/(1+(x/(X/2))**2)**2)*(2/X)
...
# Extract simulated intensity profile at target plane
Int = F.get_intensity()
